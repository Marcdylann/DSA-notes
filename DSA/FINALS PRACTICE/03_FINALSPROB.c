...
int main( ) {	
  ...
  VHeap H;
  BST T = createMagicalBSTvheap(&H);

  displayVHeap(H);

  ArrayList L = convertBstToList(&T, &H);
...
void displayVHeap(VHeap V) {
	...
	for (int i = 0; i < VH_SIZE; i++) {
        printf("\n%10d", i);
        if (strcmp(V.VH_node[i].elem.prodID, "") == 0) { // If the node is empty
            printf("%10s%15s%15s", "EMPTY", "N/A", "N/A");
        } else {
            printf("%10s%15d%15d", 
...
ArrayList convertBstToList(BST *B, VHeap *VH) {   //Removes from the BST smallest element to the biggest until BST is empty 
    ...
    while(*B != -1){
      ...
      while(VH->VH_node[current].LC != -1){
        parent = current;
        current = VH->VH_node[current].LC;
      }

      list.prod[++list.last] = VH->VH_node[current].elem;

      if(parent != -1){
        VH->VH_node[parent].LC = VH->VH_node[current].RC;
      } else {
        *B = VH->VH_node[current].RC;
      }

      VH->VH_node[current].RC = VH->avail;
      VH->avail = current;
    }
...
void displayList(ArrayList A) {
  ...
  printf("\n\nContents of the ArrayList :: ");
    printf("\n----------------------------");
    printf("\n%-8s%-10s%-6s%-10s%-6s", "ProdID", "Name", "Wt(g)", "Price", "Qty");
    printf("\n%-8s%-10s%-6s%-10s%-6s", "------", "----", "-----", "-----", "---");
