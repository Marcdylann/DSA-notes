 ...
 *------------------------------------------------------------------------------------*/
    ...
    directedAdjList L = populateDirectedAdjList();
    displayAdjList(L);
...
labeledAdjMatrix convertAdjListToMatrix (directedAdjList L) {
    ...
    for(i = 0; i < MAX_VERTEX; i++){
      for(trav = L.head[i]; trav != NULL; trav = trav->link){
          M.matrix[i][trav->info.adjVertex - 'A'] = trav->info.weight;
          M.edgeCount++;
      }
    }
